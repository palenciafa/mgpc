    ...
    <div class="min-h-screen bg-slate-900/90 px-6 py-8">
                                ...
                                <tr class="hover:bg-slate-700/30 transition-colors duration-200">
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="flex space-x-2">
                                            ...
                                            <!-- Delete Button -->
                                            <form action="{{ route('categories.destroy', $supplier) }}" method="POST"
                                                class="inline">
                                                @csrf @method('DELETE')
                                                <button type="submit"
                                ...
                                <tr>
                                    <td colspan="2" class="px-6 py-12 text-center">
                                        <div class="flex flex-col items-center">
                                            ...
                                            <p class="text-slate-500 text-sm mb-4">Get started by creating your first category.
                                            </p>
                                            <a href="{{ route('suppliers.create') }}"
        ...
        <div id="detailsModal" class="hidden fixed inset-0 bg-black/70 flex items-center justify-center z-50">
            <div class="bg-slate-800 rounded-xl shadow-lg w-full max-w-md p-6 relative">
                ...
                <div id="modalContent" class="space-y-3 text-slate-300">
                    <!-- Dynamic content will be injected here -->
                </div>
            </div>
        </div>
        <script>
            const suppliers = @json($suppliers); // Pass suppliers data to JS

            function openModal(id) {
                const supplier = suppliers.find(s => s.id === id);
                if (!supplier) return;

                // Fill modal with supplier info
