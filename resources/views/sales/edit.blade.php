...
@section('content')
    ...
    <div class="min-h-screen px-6 py-8">
        <div class="max-w-3xl mx-auto">
            <!-- Header -->
            <div class="mb-8">
                <h1 class="text-3xl font-bold text-white mb-2">Edit Sale</h1>
                <p class="text-slate-400">Update the sale information below.</p>
            </div>
            ...
            <div class="bg-slate-800/50 backdrop-blur-xl rounded-xl border border-slate-700/50 p-6">
                <form action="{{ route('sales.update', $sale) }}" method="POST" class="space-y-6">
                    ...
                    <div>
                        ...
                        <input type="number" name="amount" id="amount" step="0.01" value="{{ old('amount', $sale->total_price) }}" readonly
                            class="w-full px-4 py-2 rounded-lg bg-slate-700/50 text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500 transition duration-200">
                    </div>
                    ...
                    <div class="bg-slate-700/30 rounded-lg p-4 border border-slate-600/30">
                        <p class="text-slate-300 text-sm">
                            <strong class="text-white">Created on:</strong> {{ $sale->created_at->format('M d, Y - H:i') }}
                        </p>
                        <p class="text-slate-300 text-sm">
                            <strong class="text-white">Created by:</strong> {{ $sale->user->name ?? 'N/A' }}
                        </p>
                    </div>
